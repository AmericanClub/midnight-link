...
- **Frontend Admin Console → "Payment Partners"** (`AdminConsole.jsx`): list, New partner dialog (credentials-once reveal + copy), Manage dialog (edit webhook, rotate key/secret, activate/deactivate, delete, stats, charges table, webhook delivery log, resend). Nav `partners`. Testids: new-partner-btn, partner-name-input, partner-hook-input, partner-create-submit, partner-new-apikey/secret, partner-row-*, partner-manage-*, partner-hook-edit/save, partner-rotate-key, partner-toggle-active, partner-delete, partner-resend-*.
