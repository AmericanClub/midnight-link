...
- **Frontend Admin Console → "Payment Partners"** (`AdminConsole.jsx`): list view with summary stats (partners / total charges / total collected) + New partner dialog (credentials-once reveal + copy). Clicking a partner opens a **full-width detail view** (not a modal) with stats + **Tabs: Charges | Deliveries | Settings**. Charges tab has status filter chips (All/Paid/Pending/Expired) + reference search + **server-side pagination** + resend. Deliveries tab has success/failed filter + pagination. Settings tab has webhook edit, rotate key/secret, activate/deactivate, delete. Backend adds paginated `GET /{id}/charges?status=&q=&page=&limit=` and `GET /{id}/deliveries?status=&page=&limit=` (detail endpoint now returns partner + stats only). Redesigned (2026-06) so it scales when data grows. Testids: partner-detail, partner-back, tab-charges/deliveries/settings, filter-*, charge-search, pager-prev/next, partner-hook-edit/save, partner-rotate-key, partner-toggle-active, partner-delete, partner-resend-*.
...
- **Tested:** iteration_17.json — backend 19/19 pytest PASS, frontend 100%. Real payment completion not auto-testable (needs real money); paid→signed-webhook→delivery path pre-verified manually (resend to httpbin = 200 + logged). Regression suite: `backend/tests/test_iteration_17_partners.py`.
